...
#perform threading
t1 = threading.Thread(target=screenshot.print_screen, name='t1') 
t2 = threading.Thread(target=response1.sendmail1, name='t2') 
t3 = threading.Thread(target=response2.response, name='t3') 
...
while(True):
    ...
    if (GetClassName(data) == 'gun') and (c == 0):
       c=1
